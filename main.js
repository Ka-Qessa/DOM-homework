...
for (var i = 1; i <= 16; i += 1) {
  ...
  hole.id = "hole".concat(i);
  gameBoard.appendChild(hole);
}
var character = document.createElement('div');
character.classList.add('hole_has-mole');
gameBoard.appendChild(character);
var randomHole = document.querySelectorAll('.hole');
var randomPosition = Math.floor(Math.random() * randomHole.length);
var currentHole = randomHole[randomPosition];
function moveCharacter() {
  var newRandomPosition = Math.floor(Math.random() * randomHole.length);
  var newHole = randomHole[newRandomPosition];
